...
class TikTokLiveClient(AsyncIOEventEmitter):
    ...
    def __init__(
        ...
        self._room_title: Optional[str] = None
    ...
    async def start(
        ...
        if self._ws.connected:
            raise AlreadyConnectedError(
                "You can only make one connection per client!")
        ...
        try:
            ...
            # Get Room Title
            self._room_title: str = str(await self._web.fetch_room_title("html"))
            ...
            try:
                self._logger.error(
                    "Failed to parse room ID from HTML. Using API fallback.")
                self._room_id: int = int(await self._web.fetch_room_id_from_api(self.unique_id))

                # Get Room Title
                self._room_title: str = await self._web.fetch_room_title("api")
    ...
    async def connect(
        ...
        try:
            ...
            self._logger.debug(
                "The client has been manually stopped with 'client.stop()'.")
    ...
    async def disconnect(self, close_client: bool = False) -> None:
        ...
        if self._event_loop_task is not None:
            try:
                ...
                self._logger.debug(
                    "an exception in event loop is ignored", exc_info=True)
    ...
    async def _ws_client_loop(
        ...
        async for webcast_response in self._ws.connect(
            ...
            async for event in self._parse_webcast_response(webcast_response):
                self._logger.debug(
                    f"Received Event '{event.type}' [{event.size} bytes]")
    ...
    async def _parse_webcast_response_message(self, webcast_response_message: Optional[
        ...
        if webcast_response_message is None:
            self._logger.warning(
                "Received a null ProtoMessageFetchResultMessage from the Webcast server.")
            return []

        # Get the proto mapping for proto-events
        event_type: Optional[Type[ProtoEvent]] = EVENT_MAPPINGS.get(
            webcast_response_message.method)
        response_event: Event = WebsocketResponseEvent(
        ).from_dict(webcast_response_message.to_dict())
    ...
    async def _resolve_user_id(self, unique_id: str | int) -> str:
        ...
        if parsed_id.isdigit() and self._is_userid:
            ...
            if not resolved_id:
                raise FailedResolveUserId(
                    f"Resolved ID is invalid: {resolved_id}")
            return resolved_id
        return parsed_id
