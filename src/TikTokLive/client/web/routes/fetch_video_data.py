...
class FetchVideoDataRoute(ClientRoute):
    ...
    def __init__(self, web: TikTokHTTPClient):
        ...
        self._recording_started_at: Optional[int] = 0
        self._ffmpeg_config: Optional[dict] = {}
    ...
    @property
    def recording_started_at(self) -> Optional[int]:
    ...
    def __call__(
        ...
        self._logger.info(
            f"Attempting to start download on stream for '{unique_id}'.")

        if self._ffmpeg is not None:
            raise DuplicateDownloadError(
                "You are already downloading this stream!")

        record_time: dict = {
            str(record_for): "-t"} if record_for and record_for > 0 else {}
        record_data: dict = json.loads(
            room_info['stream_url']['live_core_sdk_data']['pull_data']['stream_data'])
        record_url_data: dict = record_data['data'][quality.value]['main']
        record_url: str = record_url_data.get(
            record_format.value) or record_url_data['flv']
        ...
        self._ffmpeg_config = {
            ...
            "global_options": (
                list(
                    {"-y", f"-loglevel {kwargs.pop('loglevel', 'warning')}"}.union(
                        kwargs.pop('global_options', set()))
                )
            ),
            **kwargs
        }

        self._ffmpeg = FFmpeg(**self._ffmpeg_config)
        self._thread: Thread = Thread(target=functools.partial(
            self._threaded_recording, unique_id))
        self._thread.start()
    ...
    def stop(self) -> None:
        ...
        if not self.is_recording:
            self._logger.warning(
                "Attempted to stop a stream that does not exist or has not started.")
            return
    ...
    def _threaded_recording(self, unique_id: str) -> None:
        ...
        self._recording_started_at = int(datetime.utcnow().timestamp())
        try:
            if not self._ffmpeg:
                self._ffmpeg = FFmpeg(**self._ffmpeg_config)
