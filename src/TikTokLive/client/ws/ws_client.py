...
class WebcastWSClient:
    ...
    def __init__(
        ...
        self._ws_proxy: Optional[WebcastProxy] = ws_proxy or ws_kwargs.get(
            "proxy")
        self._connect_generator_class: Union[Type[WebcastConnect], Type[WebcastProxyConnect]
                                             ] = WebcastProxyConnect if self._ws_proxy else WebcastConnect
    ...
    async def send(self, message: Union[bytes, Message]) -> None:
        ...
        if not self.connected:
            self._logger.warning(
                "Attempted to send a message without an open WebSocket connection.")
            return
    ...
    def get_ws_cookie_string(self, cookies: httpx.Cookies) -> str:
        ...
        if session_id:
            ...
            redacted_cookie_string = cookie_string.replace(
                session_id, redacted_sid)
    ...
    async def connect(
        ...
        if self._ws_proxy is not None:
            ws_kwargs["proxy_conn_timeout"] = ws_kwargs.get(
                "proxy_conn_timeout", 10.0)
        ...
        self._connection_generator: WebcastConnect = self._connect_generator_class(
            ...
            logger=self._logger,
            # Always *should* be none as we build this internally
            uri=ws_kwargs.pop('uri', None),
            base_uri_append_str=(ws_kwargs.pop(
    ...
    async def _ping_loop_fn(self, room_id: int) -> None:
        ...
        try:
            ...
            if self._connection_generator is not None and self._connection_generator.ws_options is not None:
                ping_interval = float(self._connection_generator.ws_options.get(
                    "ping-interval", ping_interval))

            # Create the heartbeat message (it is always the same)
            hb_message: bytes = bytes(
                HeartbeatFrame.from_defaults(room_id=room_id))
        ...
        try:
            self._logger.debug(
                f"Starting ping loop with interval of {ping_interval} seconds.")
