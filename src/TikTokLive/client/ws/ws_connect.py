...
"""
WebcastIterator: Type = AsyncIterator[Tuple[Optional[WebcastPushFrame],
        ...
        while True:
            try:
                ...
                async with self as protocol:
                    self._ws = protocol
                    self._ws_options = extract_websocket_options(
                        self._ws.response_headers)
                    ...
                    async for payload_bytes in protocol:
                        ...
                        if webcast_push_frame.payload_type != "msg":
                            webcast_push_frame.payload = extract_webcast_response_message(
                                webcast_push_frame, logger=self._logger)
                            self._logger.debug(
                                f"Received payload of type '{webcast_push_frame.payload_type}', not 'msg': {webcast_push_frame}")
                            continue

                        # If it is of type msg, we can extract the ProtoMessageFetchResult item within
                        webcast_response: ProtoMessageFetchResult = extract_webcast_response_message(
                            webcast_push_frame, logger=self._logger)
...
class WebcastProxyConnect(ProxyConnect, WebcastConnect):
    ...
    def __init__(
        ...
        super().__init__(
            proxy=self._convert_proxy(proxy) if isinstance(
                proxy, httpx.Proxy) else proxy,
            **kwargs
        )
    ...
    def _convert_proxy(cls, proxy: httpx.Proxy) -> websockets_proxy.Proxy:
        ...
        parsed: Tuple[ProxyType, str, int, Optional[str],
                      Optional[str]] = parse_proxy_url(str(proxy.url))
